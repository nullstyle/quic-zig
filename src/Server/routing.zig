...
pub fn installLbConfig(server: *Server, new_cfg: lb_mod.LbConfig) Error!void {
    new_cfg.validate() catch return Error.InvalidConfig;
    if (server.lb_factory == null) return Error.InvalidConfig;
    if (new_cfg.cidLength() != server.local_cid_len) return Error.InvalidConfig;
    ...
    if (server.stateless_reset_key != null) {
...
pub fn rotateLiveSlotCids(server: *Server) usize {
    const factory_ptr = if (server.lb_factory) |*f| f else return 0;
    const key = server.stateless_reset_key orelse return 0;
    ...
    for (server.slots.items) |slot| {
        ...
        const cid_slice = cid_buf[0..server.local_cid_len];
        ...
        resyncSlotCids(server, slot) catch continue;
        rotated += 1;
    }
    return rotated;
}

pub fn findSlotForDatagram(server: *Server, bytes: []const u8) ?*Slot {
    const dcid = peekDcidForServer(bytes, server.local_cid_len) orelse return null;
    const key = cidKeyFromSlice(dcid);
    return server.cid_table.get(key);
}

// Doc comment lives on the `Connection.mintLocalScid` thunk in Connection.zig.
pub fn mintLocalScid(server: *Server, dst: []u8) Error!void {
    if (server.lb_factory) |*factory| {
...
pub fn resyncSlotCids(server: *Server, slot: *Slot) Error!void {
    ...
    while (i < slot.tracked_cid_count) {
        ...
        if (!containsConnectionId(snapshot, tracked)) {
            _ = server.cid_table.remove(cidKeyFromConnectionId(tracked));
    ...
    for (snapshot) |cid| {
        ...
        const gop = try server.cid_table.getOrPut(server.allocator, cidKeyFromConnectionId(cid));
...
pub fn dropAllCidsFromTable(server: *Server, slot: *Slot) void {
    removeCidIfOwnedBy(server, slot.initial_dcid, slot);
    for (slot.tracked_cids[0..slot.tracked_cid_count]) |cid| {
        removeCidIfOwnedBy(server, cid, slot);
    }
...
fn removeCidIfOwnedBy(server: *Server, cid: ConnectionId, slot: *Slot) void {
    const key = cidKeyFromConnectionId(cid);
    if (server.cid_table.getPtr(key)) |vp| {
        if (vp.* == slot) _ = server.cid_table.remove(key);
    }
}
...
pub fn maybeReplenishConnectionIds(server: *Server, slot: *Slot) void {
    if (!server.auto_replenish_connection_ids) return;
    if (slot.cids_replenished) return;
    const key = if (server.stateless_reset_key) |*k| k else return;
    ...
    const budget = @min(
        @min(
            ...
            @as(usize, server.max_auto_replenish_cids),
        ),
        provisions.len,
    );
    if (budget == 0) return;
    ...
    var n: usize = 0;
    while (n < budget) : (n += 1) {
        const cid = cid_storage[n][0..server.local_cid_len];
        mintLocalScid(server, cid) catch return;
