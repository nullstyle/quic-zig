...
pub fn handleNewConnectionId(
    ...
    try conn.registerPeerCid(0, nc.sequence_number, nc.retire_prior_to, cid, nc.stateless_reset_token);
}
...
pub fn handleRetireConnectionId(
    ...
    // datagram.
    conn.incoming_retire_cid_count +|= 1;
    if (conn.incoming_retire_cid_count > incoming_retire_cid_cap) {
        conn.close(true, transport_error_protocol_violation, "retire_connection_id flood");
        return;
    }
    ...
    // protocol violation.
    if (conn.current_incoming_local_cid_seq) |incoming_seq| {
        if (rc.sequence_number == incoming_seq) {
            conn.close(true, transport_error_protocol_violation, "retire_connection_id refers to receiving CID");
            return;
        }
    }
    ...
    if (conn.paths.getConst(0)) |path| {
        if (rc.sequence_number >= path.next_local_cid_seq) {
            conn.close(true, transport_error_protocol_violation, "retire_connection_id sequence not yet issued");
            return;
        }
    }
    ...
    if (conn.smallestLiveLocalCidSeq(0)) |smallest| {
        if (rc.sequence_number < smallest) return;
    }
    conn_cids.retireLocalCidFromPeer(conn, 0, rc.sequence_number);
    conn_cids.dropPendingLocalCidAdvertisement(conn, 0, rc.sequence_number);
}
...
pub fn handleNewToken(conn: *Connection, nt: frame_types.NewToken) void {
    if (conn.role != .client) {
        ...
        conn.close(true, transport_error_protocol_violation, "new_token from peer to server");
        return;
    }
    if (nt.token.len == 0) {
        // Zero-length NEW_TOKEN is FRAME_ENCODING_ERROR per §19.7.
        conn.close(true, transport_error_frame_encoding, "zero-length new_token");
        return;
    }
    if (conn.new_token_callback) |cb| cb(conn.new_token_user_data, nt.token);
}
