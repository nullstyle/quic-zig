//! Echo server written directly against `Server` / `Connection` —
//! the explicit "raw" variant of `echo_server.zig`, kept as the
//! teaching artifact for what `quic.app.Driver` does for you.
//!
//! One process, one UDP socket, real loopback traffic:
//!
//! ```sh
//! zig build examples
//! ./zig-out/bin/echo-server-raw-example           # listens on 127.0.0.1:4433
//! ./zig-out/bin/echo-client-example               # round-trips against it
//! ```
//!
//! Start with `echo_server.zig` for the application-layer shape to
//! copy; read this one when you need to see the machinery: the
//! slot-iterator connection walk, the `pollEvent` switch, the
//! hand-rolled stream table, and the short-write staging the Driver
//! would otherwise own. The shape to copy for your own server:
//!
...
/// to its `initial_max_streams_bidi` — pinned by the comptime
/// assert below so the two can never silently drift apart (a
/// conforming peer could otherwise open a stream this example
/// never services: a wire-visible hang, not an error).
...
/// tail — pinned by the comptime assert below. (`receiveDatagramInfo`
/// reports the truncation via `payload_len > len`, and
/// `nextDatagramSize` peeks the required size.)
const datagram_chunk_bytes: usize = 2048;

comptime {
    std.debug.assert(datagram_chunk_bytes >= common.transportParams().max_datagram_frame_size);
}
